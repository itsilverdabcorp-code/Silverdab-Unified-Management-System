// app/Modules/tripbooking/FleetAllTrips/FleetAllTripsPage.web.tsx
//
// Fleet Ops — All Trips (web build).
...
function StatusBadge({
  ...
  return (
    ...
    >
      {config.dot && (
        <span style={{ backgroundColor: config.dot, width: 5, height: 5, borderRadius: "50%" }} className="inline-block flex-shrink-0" />
      )}
      {config.label}
    </span>
  );
...
function Truncated({
  ...
  return (
    <span className="relative inline-block max-w-full align-bottom" onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}>
