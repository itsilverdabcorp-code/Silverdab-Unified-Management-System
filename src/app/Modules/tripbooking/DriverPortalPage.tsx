...
const DUTY_STATUS_OPTIONS: { key: DriverDutyStatus; label: string; color: string }[] = [
...
async function runDriverAction(trip: FleetTrip, next: TripStatus): Promise<void> {
  if (next === "ongoing") return startFleetTrip(trip.id);
...
function StatCard({ value, label, mono, theme }: { value: string | number; label: string; mono?: boolean; theme: any }) {
  return (
    <View
      style={{
        flex: 1,
...
function DutyStatusSelector({
  ...
  return (
    <View style={{ marginBottom: 16 }}>
      ...
      <View style={{ flexDirection: "row", gap: 8, opacity: disabled ? 0.5 : 1 }}>
        {DUTY_STATUS_OPTIONS.map((opt) => {
          ...
          return (
            ...
            >
              {busy && active ? (
                <ActivityIndicator size="small" color={active ? "#fff" : opt.color} />
              ) : (
                <Text
                  style={{
                    fontFamily: "Outfit-medium",
      ...
      {locked && (
        ...
        >
          You're on a trip right now — duty status is locked until it's completed.
        </Text>
      )}
    </View>
  );
  ...
  return (
    ...
    >
      <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "flex-start", gap: 10 }}>
        <View style={{ flex: 1 }}>
          <Text style={{ fontFamily: "Outfit-medium", fontSize: 14, color: theme.textActive ?? theme.text }}>
            {trip.pickupLabel} → {trip.dropoffLabel}
          </Text>
          <Text style={{ fontFamily: "Outfit", fontSize: 11.5, color: theme.subtext, marginTop: 3 }}>
            {formatSchedule(trip.departureDatetime)} · {trip.passengerCount} passenger
      ...
      {trip.requestorName ? (
        <Text style={{ fontFamily: "Outfit", fontSize: 11.5, color: theme.subtext, marginTop: 8 }}>
      ...
      {trip.vehiclePlate ? (
        <View style={{ flexDirection: "row", alignItems: "center", marginTop: 6 }}>
          <Text style={{ fontFamily: "Outfit", fontSize: 11.5, color: theme.subtext }}>Vehicle</Text>
          <View
            style={{
          ...
          >
            <Text style={{ fontFamily: "Outfit-medium", fontSize: 10.5, color: theme.textActive ?? theme.text }}>
              {trip.vehiclePlate}
            </Text>
          </View>
        </View>
      ...
      {trip.purpose ? (
        <Text style={{ fontFamily: "Outfit", fontSize: 11.5, color: theme.subtext, marginTop: 6 }}>
          {trip.purpose}
        </Text>
      ) : null}
      ...
      {action && (
        ...
        >
          {busy ? (
            ...
            <Text style={{ fontFamily: "Outfit-medium", fontSize: 13, color: "#fff" }}>{action.label}</Text>
          )}
        </TouchableOpacity>
      )}
...
export default function DriverPortalPage({ user }: Props) {
  ...
  const myDriver = useMemo(
    () => drivers.find((d) => d.name?.toLowerCase() === user.displayName?.toLowerCase()),
    [drivers, user.displayName],
  );
  const myVehicle = useMemo(
    () => (myDriver?.vehicleId ? vehicles.find((v) => v.id === myDriver.vehicleId) : undefined),
    [vehicles, myDriver],
  );

  const myTrips = useMemo(
    () => trips.filter((t) => t.driverName?.toLowerCase() === user.displayName?.toLowerCase()),
  ...
  const isOnTrip = useMemo(() => activeTrips.some(isTripInProgress), [activeTrips]);
  const currentDutyStatus: DriverDutyStatus = myDriver?.dutyStatus ?? "off_duty";
  ...
  async function handleAdvance(trip: FleetTrip) {
    ...
    try {
      await runDriverAction(trip, action.next);
      await loadAll();
    } catch (err) {
    ...
    <View style={{ flex: 1, backgroundColor: theme.background }}>
      ...
      >
        ...
        <View style={{ flexDirection: "row", alignItems: "center", gap: 12, marginBottom: 16 }}>
          ...
          >
            <Text style={{ fontFamily: "Outfit-medium", fontSize: 16, color: "#fff" }}>
          ...
          <View style={{ flex: 1, minWidth: 0 }}>
            ...
            <Text style={{ fontFamily: "Outfit", fontSize: 12, color: theme.subtext, marginTop: 1 }}>
              {myVehicle ? `Assigned to ${myVehicle.plateNumber}` : "No vehicle assigned"}
            </Text>
          </View>
        </View>
        ...
        {!myDriver ? (
          ...
          >
            <Text style={{ fontFamily: "Outfit-medium", fontSize: 13.5, color: theme.textActive ?? theme.text }}>
              You're not registered as a driver yet
            </Text>
            <Text style={{ fontFamily: "Outfit", fontSize: 12, color: theme.subtext, marginTop: 4, lineHeight: 17 }}>
              Ask your fleet dispatcher to add you under Fleet Control Tower → Drivers before trips can be
              assigned to you.
            </Text>
          </View>
        ) : (
          <>
            ...
            <View style={{ flexDirection: "row", gap: 8, marginBottom: 12 }}>
              <StatCard value={activeTrips.length} label="Active" theme={theme} />
              <StatCard value={historyTrips.length} label="Completed" theme={theme} />
              <StatCard value={myVehicle?.plateNumber ?? "—"} label="Vehicle" mono theme={theme} />
            </View>
