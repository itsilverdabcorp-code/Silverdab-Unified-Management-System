...
> = {
  ...
  completed: { label: "Completed", bg: "#e2e8f0", text: "#334155", dot: "#64748b" },
...
const ACTIVE_STATUSES: TripStatus[] = ["pending", "approved", "ongoing", "arrived", "returning"];
...
export default function FleetAllTripsPage({ user }: Props) {
  ...
  const getAvailableVehicles = useCallback(
    (excludeTripId?: string) => {
      const active = trips.filter(
        (t) => ACTIVE_STATUSES.includes(t.status) && t.status !== "pending" && t.id !== excludeTripId,
      );
      const busy = new Set(active.map((t) => (t as any).vehicleId).filter(Boolean));
      return vehicles.filter((v) => v.status !== "maintenance" && v.status !== "personal" && !busy.has(v.id));
    },
    [vehicles, trips],
  );
  ...
  const getAvailableDrivers = useCallback(
    (excludeTripId?: string) => {
      ...
      const busy = new Set(active.map((t) => (t as any).driverId).filter(Boolean));
      return drivers.filter((d) => !busy.has(d.id) && d.dutyStatus !== "personal");
    },
    [drivers, trips],
  );
      ...
      <div className="px-5 pb-5 flex-1 min-h-0">
        <div className="fatp-scroll h-full overflow-auto">
          ...
          >
            <thead>
              <tr>
                {["Pickup", "Drop-off", "Purpose", "Employee", "Vehicle", "Driver", "Schedule", "Status"].map((h) => (
                  <th
                    key={h}
                    style={{
                      color: theme.subtext,
                  ...
                  return (
                    ...
                    >
                        <td className="px-4 py-2.5">
                          <p style={{ color: theme.text }} className="text-[12.5px] font-semibold">
                            {trip.pickupLabel}
                          </p>
                        </td>
                        <td className="px-4 py-2.5">
                        ...
                        <td className="px-4 py-2.5">
                          <p style={{ color: theme.subtext }} className="text-[12px]">
                            {trip.purpose || "—"}
                          </p>
                        </td>
                        ...
                        <td className="px-4 py-2.5 whitespace-nowrap">
                          {trip.vehiclePlate ? (
                            ...
                            <span style={{ color: theme.subtext }} className="text-[12.5px]">—</span>
                          )}
                        </td>
                        <td style={{ color: theme.text }} className="px-4 py-2.5 text-[12.5px] whitespace-nowrap">
                          {trip.driverName ?? "—"}
          ...
          >
            ...
            <div className="flex flex-col gap-2.5 mb-4">
              {[
                ...
                ["Return", viewingTrip.returnDatetime ? formatDateTime(viewingTrip.returnDatetime) : "—"],
                ...
                <div key={label} className="flex items-center justify-between gap-3">
                  <span style={{ color: theme.subtext }} className="text-[11px]">{label}</span>
                  <span style={{ color: theme.text }} className="text-[12.5px] font-semibold text-right">{value}</span>
                </div>
              ))}
              {viewingTrip.status === "rejected" && viewingTrip.rejectedReason && (
                <div>
                  <span style={{ color: theme.subtext }} className="text-[11px] block mb-1">Rejection reason</span>
                  <p style={{ color: theme.text }} className="text-[12.5px]">{viewingTrip.rejectedReason}</p>
                </div>
              )}
            </div>
            ...
            {viewingTrip.status !== "pending" && (
              <div style={{ borderColor: theme.border }} className="border-t pt-3 mb-4">
                <p style={{ color: theme.text }} className="text-[12.5px] font-semibold mb-2">Status history</p>
                {!viewingTrip.statusHistory || viewingTrip.statusHistory.length === 0 ? (
                  <p style={{ color: theme.subtext }} className="text-[11px]">No history recorded for this trip yet.</p>
                ) : (
            ...
            {viewingTrip.status === "pending" && (
              <div style={{ borderColor: theme.border }} className="border-t pt-4 mb-4">
                <p style={{ color: theme.text }} className="text-[12.5px] font-semibold mb-2">Assign vehicle &amp; driver</p>
                <div className="flex flex-col gap-2">
                  <select
                ...
                {rowError[viewingTrip.id] && (
                  <p style={{ color: "#dc2626" }} className="text-[11px] mt-2">{rowError[viewingTrip.id]}</p>
                )}
              </div>
            )}
            ...
            {(viewingTrip.status === "approved" || viewingTrip.status === "arrived") && (
              <div style={{ borderColor: theme.border }} className="border-t pt-4 mb-4">
                {!reassignOpen[viewingTrip.id] ? (
                  ...
                  <>
                    <p style={{ color: theme.text }} className="text-[12.5px] font-semibold mb-2">Reassign vehicle &amp; driver</p>
                    <div className="flex flex-col gap-2">
                      <select
                        value={assignDrafts[viewingTrip.id]?.vehicleId ?? ""}
                        onChange={(e) => setDraft(viewingTrip.id, { vehicleId: e.target.value })}
            ...
            {(() => {
              ...
              if (viewingTrip.status === "approved" || viewingTrip.status === "ongoing") {
                return (
                  <div className="flex flex-col gap-2.5">
                    <button
                      onClick={async () => {
