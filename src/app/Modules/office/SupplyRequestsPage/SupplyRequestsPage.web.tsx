...
function worstStockStatus(
  ...
  if (items.some((i: SupplyRequest["items"][number]) => effective(i) === "out_of_stock")) return "out_of_stock";
  if (items.some((i: SupplyRequest["items"][number]) => effective(i) === "low")) return "low";
  return "available";
}
...
function effectiveItems(items: SupplyRequest["items"]) {
  const anyReviewed = items.some(
    (i: SupplyRequest["items"][number]) =>
      i.quantityApproved !== null && i.quantityApproved !== undefined,
  );
  if (!anyReviewed) return items;
  return items.filter((i: SupplyRequest["items"][number]) => (i.quantityApproved ?? 0) > 0);
}
...
function DetailDrawer({
  ...
  return (
      ...
      { label: "Total qty", value: String(totalQty) },
    ].map((row: { label: string; value: string }, i: number, arr: unknown[]) => (
              <div
                key={row.label}
          ...
          <div>
            ...
            <div className="space-y-2">
              {effectiveItems(request.items).map((item: SupplyRequest["items"][number], i: number) => (
                <div
                  key={`${item.itemId}-${i}`}
                  style={{
                    borderColor: theme.border,
          ...
          {trail.length > 0 && (
            <div>
              ...
              <div className="space-y-2">
                {trail.map((t: { label: string; value: string; at?: string }, i: number) => (
  ...
  const loadRequests = useCallback(async () => {
    ...
    try {
      const [data, items] = await Promise.all([
        getAllSupplyRequests(),
        getAllInventoryItems().catch((err: unknown) => {
      ...
      setLiveStock(
        Object.fromEntries(
          items.map((it: { id: string; stockStatus: string }) => [
            it.id,
            toRequestStockStatus(it.stockStatus),
          ]),
        ),
      );
      setError("");
    ...
    >
      ...
      <div className="flex-shrink-0 px-4 pt-4 pb-0">
        ...
        <div
          ...
          className="flex items-end gap-0 mb-3"
        >
          {[
