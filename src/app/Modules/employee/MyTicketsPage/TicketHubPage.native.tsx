...
const STAT_TILE_CONFIG = {
  pending: { bg: "#FFA23B", darkBg: "#FFA138" },      // orange
  inProgress: { bg: "#73B0FF", darkBg: "#6EADFF" },   // blue
  completed: { bg: "#50C781", darkBg: "#5CFF9F" },    // green
  total: { bg: "#8F8BF5", darkBg: "#ABA8FF" },        // purple
} as const;
...
function StatTile({ icon, label, value, sub, bg, isDark, darkBg }: StatTileProps) {
  const textColor = isDark ? "#0F1C22" : "#FFFFFF";
  const subTextColor = isDark ? "rgba(15,28,34,0.65)" : "rgba(255,255,255,0.85)";
  return (
    <View
      style={{
        flex: 1,
      ...
      {open && (
        <View style={{ marginBottom: 4 }}>
          {canReserveRoom &&
            pillAction(
              "Room Reservation",
              <MonitorSmartphone size={15} color="#fff" />,
              onNewRoomReservation,
              "room",
            )}
          {canBookTrip &&
            pillAction("Book Trip", <Car size={15} color="#fff" />, onNewTrip, "trip")}
          {pillAction(
            "Request Supply",
            ...
            {TABS.map((tab) => {
              ...
              return (
                ...
                >
                  ...
                  <View
                    style={{
                      backgroundColor: TAB_BADGE_COLOR[tab],
                      borderRadius: 100,
                      paddingHorizontal: 6,
                      paddingVertical: 1,
                      minWidth: 20,
  ...
  return (
    ...
    >
      <View style={{ padding: 20 }}>
        ...
        {/* Single-column ticket list / HR form */}
        <View style={{ minHeight: 400 }}>
          {step === 1 && (
            <StatStrip counts={counts} theme={theme} isDark={theme.mode === "dark"} />
          )}
          {renderRightPanel()}
        </View>
      </View>
